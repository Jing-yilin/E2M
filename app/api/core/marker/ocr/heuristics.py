...
from api.core.marker.ocr.utils import alphanum_ratio
from api.core.marker.schema.bbox import rescale_bbox, box_intersection_pct
...
def detect_bad_ocr(
    text, space_threshold=0.7, newline_threshold=0.6, alphanum_threshold=0.3
):
    if len(text) == 0:
    ...
    if invalid_chars > max(4.0, len(text) * 0.03):
        return True
...
def detected_line_coverage(page: Page, intersect_thresh=0.5, detection_thresh=0.65):
    found_lines = 0
    for detected_line in page.text_lines.bboxes:
        ...
        detected_bbox = rescale_bbox(
            page.text_lines.image_bbox, page.bbox, detected_bbox
        )
